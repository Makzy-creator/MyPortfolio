...
/** The list of phrases */
const words = ["FloatWriter", "Helloworld", "HappyPeople"];
...
export default function FloatWriter() {
  ...
  return (
    <div className="relative">
      <h1 className="relative invisible">FloatWriter</h1>
      <AnimatePresence>
        {words.map((word, index) => {
          return (
            index == currentIndex &&
            <h1
              className="absolute left-0 top-0 [letter-spacing:5px] font-semibold text-2xl"
              key={index}
            >
